...
alter table agency add constraint phone_number check (phone_number is not null);
...
alter type house_type rename to marital_status;
alter table customer rename column family_status  to marital_status;
...
select o.first_name as owner_first_name ,o.last_name as owner_last_name,h.* from owners o inner join houses h on o.id=h.owner_id order by length(o.first_name) desc limit 1;
-- - 2. Уйлордун баалары 1500, 2000 дин аралыгында бар болсо true чыгар, жок болсо false чыгар.
select h.house_type,h.price,h.furniture from houses h where h.price  between 1500 and 2000;
...
select h.*,concat(o.first_name,' ',o.last_name) as owner_name,c.first_name as customer_name,a.name as agency_name from houses h inner join owners o on h.owner_id=o.id inner join rent_info r on o.id=r.owner_id inner join customers c on r.customer_id=c.id inner join  agencies a on r.agency_id=a.id;
-- - 5. Клиенттердин 10-катарынан баштап 1999-жылдан кийин туулган 15 клиентти чыгар.
select * from customers c where extract(year from c.date_of_birth)>1999 order by date_of_birth  limit 15;
-- - 6. Рейтинги боюнча уйлорду сорттоп, уйлордун тайптарын, рейтингин жана уйдун ээлерин чыгар. (asc and desc)
select * from houses h inner join owners o on h.owner_id=o.id order by h.rating;
select * from houses h inner join owners o on h.owner_id=o.id order by h.rating desc;
-- - 7. Уйлордун арасынан квартиралардын (apartment) санын жана алардын баасынын суммасын чыгар.
select h.house_type,count(h.house_type) as houses_count ,sum(h.price) from houses h where h.house_type ='Apartment' group by h.house_type;
...
select * from agencies a inner join addresses adres on a.address_id=adres.id inner join houses h on adres.id=h.address_id where a.name='My House';
-- - 9. Уйлордун арасынан мебели бар уйлорду, уйдун ээсин жана уйдун адрессин чыгар.
select * from addresses a inner join houses h on a.id=h.address_id inner join owners o on h.owner_id=o.id where h.furniture='true';
...
select c.nationality,count(c.*) from customers c group by c.nationality;
...
select * from houses h left join  rent_info r on h.id= r.house_id left join customers c on c.id=r.customer_id ;
...
select * from houses h inner join owners o on h.owner_id;
...
select c.gender,count(c.*) from customers c group by c.gender;
-- - 21.Эн коп моонотко ижарага алынган уйду чыгарыныз.
select * from houses h inner join rent_info ri on h.id = ri.house_id order by ri.check_in limit 1 and ;
-- - 22.Эн кымбат уйду жана анын ээсин чыгарыныз.
select * from houses h inner join owners o on h.owner_id=o.id order by h.price  limit 1;
